...
import { User, Mail, Building, Phone, Monitor, CheckCircle, MousePointer, AlertCircle, ArrowRight, ChevronDown } from 'lucide-react';
...
const SolicitudPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setStatus({ loading: false, type: 'error', message: 'Error al enviar. Inténtalo de nuevo.' });
        }
    };
                ...
                <div className="w-full relative z-20 flex justify-center lg:justify-end">
                    <div className="relative w-full max-w-md rounded-2xl bg-slate-900/90 border border-white/10 shadow-2xl backdrop-blur-xl p-6 lg:p-8">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4 relative z-10" noValidate>
                            ...
                            {status.message && (
                                <div className={`flex items-center justify-center gap-2 p-2 rounded-lg text-xs font-bold animate-in fade-in slide-in-from-bottom-2 duration-300 ${status.type === 'error' ? 'text-red-400 bg-red-900/20 border border-red-900/30' : 'text-green-400 bg-green-900/20 border border-green-900/30'}`} role="alert">
